...
def make_delicatissimo(collection):
    ...
    rmakers.attach_span_beams_to_runs_across_leaf_lists([abjad.select.leaves(tuplet)])
...
def make_dotted_eighths(collection):
    ...
    voice = abjad.Voice([tuplet], name="Temporary")
    rmakers.attach_beams_to_runs_by_leaf_list([tuplet[:]], beam_lone_notes=True)
    voice[:] = []
    return tuplet, 8
...
def make_graced_tuplets(collections):
    ...
    rmakers.attach_span_beams_to_runs_across_leaf_lists(groups, beam_lone_notes=True)
...
def make_ovoid(collection):
    ...
    rmakers.attach_span_beams_to_runs_across_leaf_lists([group])
    rmakers.attach_beams_to_runs_by_leaf_list([_[:] for _ in bgcs if _ is not None])
    voice[:] = []
    return tuplet
...
def make_passepied(collection):
    ...
    tuplet = baca.from_collection(collection, [1], 32)
    voice = abjad.Voice([tuplet], name="Temporary")
    rmakers.attach_span_beams_to_runs_across_leaf_lists(
        [abjad.select.leaves(tuplet)], beam_lone_notes=True
    )
    baca.rhythm.attach_bgcs(bgcs, tuplet)
